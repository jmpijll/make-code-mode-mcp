...
/** OpenAPI security requirement — map from scheme name to list of required scopes. */
...
export interface IndexedOperation {
  ...
  operationId: string;
  /** First tag, used for namespace grouping (lowercased, camelCased) */
  primaryTag: string;
  /** All tags */
  tags: string[];
  ...
  deprecated: boolean;
  /**
   * Required scopes for this operation — one inner array per acceptable
   * security scheme. Used by the HTTP client to enrich 401/403 errors.
   */
...
export interface ProcessedSpec {
  ...
  sourceUrl: string;
  /** Application version reported by the source (e.g. "1.0.0") */
  version: string;
  /** Friendly title */
  title: string;
  /** Server prefix to prepend to operation paths (e.g. "/api/v2"). Usually empty for Make. */
  serverPrefix: string;
